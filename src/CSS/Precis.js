...
import PrecisFetchData from "./PrecisFetchData";
...
function Precis() {
        ...
        ref1.getDownloadURL().then((fileUrl) => {
          ...
          uploadTask2.on("state_changed", console.log, console.error, () => {
            ref2.getDownloadURL().then((imageUrl) => {
              ...
              db.collection("precisandcomposition").add({
  ...
  function getDatas() {
    db.collection("precisandcomposition")
  ...
  return (
    <div className="editorialmain">
      <div className="editorialdisplay">
        <h2 style={{marginLeft:"50px",marginBottom:"30px"}}>Precis And Composition</h2>
        {list.map((links) => {
          const { title, description, id, date } = links;
          return (
           <PrecisFetchData title={title} description={description} id={id} date={date} />
