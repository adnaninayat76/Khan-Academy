...
import PublicFetchData from "./PublicFetchData";
...
function Public() {
        ...
        ref1.getDownloadURL().then((fileUrl) => {
          ...
          uploadTask2.on("state_changed", console.log, console.error, () => {
            ref2.getDownloadURL().then((imageUrl) => {
              ...
              db.collection("publicadministration").add({
  ...
  function getDatas() {
    db.collection("publicadministration")
  ...
  return (
    <div className="editorialmain">
      <div className="editorialdisplay">
        <h2 style={{marginLeft:"50px",marginBottom:"30px"}}>Public Administration</h2>
        {list.map((links) => {
          const { title, description, id, date } = links;
          return (
           <PublicFetchData title={title} description={description} id={id} date={date} />
