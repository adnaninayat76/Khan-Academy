...
import PoliticalScience1FetchData from "./PoliticalScience1FetchData";
...
function PoliticalScience1() {
        ...
        ref1.getDownloadURL().then((fileUrl) => {
          ...
          uploadTask2.on("state_changed", console.log, console.error, () => {
            ref2.getDownloadURL().then((imageUrl) => {
              ...
              db.collection("politicalscience").add({
  ...
  function getDatas() {
    db.collection("politicalscience")
  ...
  return (
    <div className="editorialmain">
      <div className="editorialdisplay">
        <h2 style={{marginLeft:"50px",marginBottom:"30px"}}>Political Science I</h2>
        {list.map((links) => {
          const { title, description, id, date } = links;
          return (
           <PoliticalScience1FetchData title={title} description={description} id={id} date={date} />
...
export default PoliticalScience1;
