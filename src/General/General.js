...
import GeneralFetchData from "./GeneralFetchData";
        ...
        ref1.getDownloadURL().then((fileUrl) => {
          ...
          uploadTask2.on("state_changed", console.log, console.error, () => {
            ref2.getDownloadURL().then((imageUrl) => {
              ...
              db.collection("generalknowledge").add({
  ...
  return (
    <div className="editorialmain">
      <div className="editorialdisplay">
        <h2 style={{marginLeft:"50px",marginBottom:"30px"}}>General Knowledge</h2>
        {list.map((links) => {
          const { title, description, id, date } = links;
          return (
           <GeneralFetchData title={title} description={description} id={id} date={date} />
