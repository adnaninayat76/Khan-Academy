...
function OpinionFetchData({id,date,description,title}) {
    ...
    const updateTodo = () => {
      db.collection("opinions")
        .doc(id)
        .set({ title: input1, description: input2 }, { merge: true });
      setOpen(false);
    };
  ...
  <Button onClick={updateTodo} color="secondary" variant="outlined" style={{marginTop:"10px"}}>Update Todo</Button>
          ...
          {/* <h1>333333333333333</h1> */}
              ...
              <article className="question" key={id}>
                ...
                <button className="mybtn"  style={{margin:"5px"}} onClick={(e) => db.collection("opinions").doc(id).delete()}>
                <HighlightOffIcon/>
               </button>
...
export default OpinionFetchData
